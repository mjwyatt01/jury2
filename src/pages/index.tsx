...
export default function JuryMessagingApp() {
  ...
  type Demographics = { occupation: string; age: string; raceEthnicity: string; gender: string; education: string };
  const [demographics, setDemographics] = useState<Record<number, Demographics>>({});
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        if (parsed.staffNotes != null) {
          if (typeof parsed.staffNotes === 'string') {
            setStaffNotes(parsed.staffNotes);
          } else if (typeof parsed.staffNotes === 'object') {
            // Legacy format: was saved as per-juror object, ignore it
            setStaffNotes('');
          }
        }
  ...
  }, [messages, approvals, strike, strikeParty, scores, staffNotes, plaintiffPeremptory, defensePeremptory, plaintiffCause, defenseCause, demographics]);
  ...
  const updateDemographic = (id: number, field: keyof Demographics, value: string) => {
    setDemographics((prev) => ({
      ...prev,
      [id]: { ...{ occupation: '', age: '', raceEthnicity: '', gender: '', education: '' }, ...prev[id], [field]: value },
    }));
  };

  const getDemo = (id: number): Demographics => demographics[id] || { occupation: '', age: '', raceEthnicity: '', gender: '', education: '' };
  ...
  useEffect(() => {
    ...
    const notesSnippet = (typeof staffNotes === 'string' ? staffNotes : '').slice(0, 800);
  ...
  /* ---------- Render ---------- */
  return (
    <div className="relative h-screen bg-background overflow-hidden text-foreground flex flex-col">
      <style jsx global>{`
      ...
      <div className="flex-shrink-0 px-4 pt-4 pb-3 border-b border-border">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-foreground tracking-tight leading-tight">{caseName}</h1>
            <p className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-wider">Jury Selection Panel</p>
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="secondary"
              onClick={() => setDarkMode(!darkMode)}
              className="bg-secondary border-border hover:bg-secondary text-muted-foreground font-semibold text-xs h-8 px-2.5"
              title={darkMode ? "Switch to light mode" : "Switch to dark mode"}
            >
              {darkMode ? <Sun className="w-3.5 h-3.5" /> : <Moon className="w-3.5 h-3.5" />}
            </Button>
            <Button variant="secondary" onClick={() => setSettingsOpen(true)} className="bg-secondary border-border hover:bg-secondary text-muted-foreground font-semibold text-xs h-8 px-2.5">
              <Settings className="w-3.5 h-3.5 mr-1.5" /> Settings
            </Button>
            <Button variant="secondary" onClick={() => setAiOpen(v => !v)} className="bg-secondary border-border hover:bg-secondary text-amber-400 font-semibold text-xs h-8 px-2.5">
              <Brain className="w-3.5 h-3.5 mr-1.5" /> AI
            </Button>
            <Button variant="secondary" onClick={downloadCSV} className="bg-secondary border-border hover:bg-secondary text-muted-foreground font-semibold text-xs h-8 px-2.5">Export CSV</Button>
      ...
      <div className="grid grid-cols-3 gap-4 h-[calc(100%-80px)] p-4">
        ...
        <div className="col-span-1 flex flex-col h-full">
          ...
          <div className="flex-shrink-0 mb-3">
            <div className="bg-card rounded-lg border border-border p-4 shadow-xl">
              <h2 className="text-sm font-bold text-muted-foreground mb-3 tracking-tight">Strike Tracker</h2>
              <div className="grid grid-cols-2 gap-3">
                {/* Plaintiff */}
                <div className="space-y-2">
                  <div className="flex items-center gap-1 mb-1">
                    <Scale className="w-3 h-3 text-blue-400" />
                    <span className="text-xs text-muted-foreground font-semibold">Plaintiff</span>
                  </div>
                  <div className="space-y-1.5">
                    <div>
                      <div className="flex items-center justify-between mb-0.5">
                        <span className="text-[10px] text-muted-foreground">Peremptory</span>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] text-muted-foreground">For Cause</span>
                      <span className="text-[10px] font-bold text-muted-foreground">{plaintiffCause}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className="text-xs text-muted-foreground font-semibold">Defense</span>
                  </div>
                  <div className="space-y-1.5">
                    <div>
                      <div className="flex items-center justify-between mb-0.5">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] text-muted-foreground">For Cause</span>
                      <span className="text-[10px] font-bold text-muted-foreground">{defenseCause}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {gridCells.map((cell, idx) => {
              ...
              if (j.deleted) {
                return (
                  <div
                    ...
                    className={`relative flex flex-col items-center justify-center text-center border rounded-lg shadow-lg border-border bg-muted/30 backdrop-blur-sm cursor-pointer hover:border-amber-500/40 hover:bg-muted/40 transition-all group ${dragOverIndex === idx ? 'ring-2 ring-amber-500' : ''}`}
                  >
                    <Plus className="w-5 h-5 text-muted-foreground group-hover:text-amber-400 transition-colors mb-1" />
                    <div className="text-muted-foreground group-hover:text-foreground text-[10px] font-semibold transition-colors">Add Juror</div>
                    <div className="text-muted-foreground group-hover:text-foreground text-[8px] mt-0.5 transition-colors">#{j.id}</div>
                  </div>
                );
              }
              ...
              return (
                ...
                >
                  ...
                  {!isStruck && unreadFlag && (
                    <div className="absolute -top-1.5 -right-1.5 w-6 h-6 bg-amber-500 rounded-full flex items-center justify-center shadow-lg border-2 border-background">
                  ...
                  <CardContent className="p-3.5 text-sm font-medium flex flex-col items-center gap-1 relative z-10 w-full">
                    ...
                    <div className="absolute top-1 left-1 text-[8px] font-bold text-muted-foreground bg-muted px-1.5 py-0.5 rounded">
                      #{j.id}
                    </div>

                    {/* Score badge - smaller, bottom-left */}
                    {!isStruck && (
                      <div className={`absolute bottom-1 left-1 text-[8px] font-bold px-1.5 py-0.5 rounded bg-muted ${getScoreColor(getScore(j.id))}`}>
                        {getScore(j.id)}
                      </div>
                    )}

                    {/* Name - prominent display */}
                    <div className="font-bold text-foreground leading-tight mt-3 text-base tracking-tight">{j.name}</div>
          ...
          {activeTab === "script" && (
            <div className="flex-1 flex flex-col min-h-0">
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-sm font-bold text-foreground tracking-tight">Vehicle Accident – Voir Dire Script</h2>
                <div className="flex gap-1.5">
                  <Button onClick={() => setScriptText(VOIR_DIRE_TEMPLATE)} className="bg-secondary border-border hover:bg-secondary text-muted-foreground font-semibold text-xs h-8 px-2.5">Reset to template</Button>
                </div>
              </div>
              <textarea
                className="w-full h-full p-3 text-xs border border-border rounded-lg resize-none bg-muted text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 font-mono leading-snug"
                value={scriptText}
          ...
          {activeTab === "juror" && selectedJuror ? (
            ...
            >
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  {editingName ? (
                    <Input
                      ...
                      autoFocus
                      className="text-base font-bold bg-muted border-amber-500 text-foreground h-7 w-48"
                    />
                  ) : (
                    <h2
                      className="text-base font-bold text-foreground hover:text-amber-400 cursor-pointer transition-colors"
                      onClick={() => {
                  ...
                  {editingNumber ? (
                    <Input
                      ...
                      autoFocus
                      type="number"
                      className="text-xs bg-muted border-amber-500 text-muted-foreground h-6 w-28"
                    />
                  ) : (
                    <span
                      className="text-xs text-muted-foreground hover:text-amber-400 cursor-pointer transition-colors"
                      onClick={() => {
              ...
              <div className="grid grid-cols-2 gap-3 flex-1 min-h-0">
                ...
                {/* LEFT COLUMN - TEAM NOTES + TEAM COMMUNICATION */}
                <div className="flex flex-col min-h-0 space-y-3">
                  {/* Team Notes */}
                  <div className="flex flex-col flex-1 min-h-0 bg-card rounded-xl border border-border p-3 shadow-xl">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <NotebookPen className="w-4 h-4 text-emerald-400" />
                        <h3 className="text-sm font-bold text-foreground">Team Notes</h3>
                    ...
                    {!notesCollapsed && (
                      <div className="flex-1 min-h-[200px]">
                        <textarea
                          className="w-full h-full p-3 text-xs border border-border rounded-lg resize-none bg-muted/30 text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 leading-snug"
                  ...
                  {/* Team Communication */}
                  <div className="flex flex-col min-h-[280px] bg-card rounded-xl border border-border p-3 shadow-xl">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <MessageSquare className="w-4 h-4 text-amber-400" />
                        <h3 className="text-sm font-bold text-foreground">Team Communication</h3>
                        <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">
                    ...
                    <div className="flex gap-1.5 mb-2">
                      ...
                      <button
                        ...
                        className={`flex-1 px-2 py-1 rounded-lg text-[10px] font-semibold transition-all ${
                          messageType === 'alert'
                            ? 'bg-amber-600 text-foreground border-2 border-amber-500'
                            : 'bg-muted/30 text-muted-foreground border border-border hover:bg-muted'
                        }`}
                      >
                        ⚠️ Alert
                      ...
                      <button
                        ...
                        className={`flex-1 px-2 py-1 rounded-lg text-[10px] font-semibold transition-all ${
                          messageType === 'flag'
                            ? 'bg-red-600 text-foreground border-2 border-red-500'
                            : 'bg-muted/30 text-muted-foreground border border-border hover:bg-muted'
                        }`}
                      >
                        🚩 Flag
                    ...
                    {/* Messages Display */}
                    <div className="flex-1 overflow-y-auto border border-border rounded-lg p-2 bg-muted/30 mb-2 space-y-1 min-h-[100px]">
                          ...
                          return (
                            <div key={i} className={`p-1.5 rounded-lg border ${messageTypeColors[m.type]}`}>
                              <div className="flex items-start justify-between mb-0.5">
                                ...
                                <span className="text-[9px] text-muted-foreground">{formatTimestamp(m.timestamp)}</span>
                              </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Input
                        ...
                        className="bg-muted border-border text-foreground placeholder-muted-foreground focus:ring-amber-500/50 focus:border-amber-500/50 h-8 text-xs"
                ...
                {/* RIGHT COLUMN - DEMOGRAPHICS + RATING + STRIKES (scrollable) */}
                <div className="flex flex-col min-h-0 overflow-y-auto space-y-3">
                  {/* Demographics */}
                  <div className="bg-card rounded-xl border border-border p-3 shadow-xl">
                    ...
                    <div className="space-y-2.5">
                      {([
                        ['occupation', 'Occupation', 'e.g., Software Engineer, Teacher'],
                        ['age', 'Age', 'e.g., 35'],
                        ['raceEthnicity', 'Race/Ethnicity', 'e.g., Caucasian, Hispanic, Asian'],
                        ['gender', 'Gender', 'e.g., Male, Female, Non-binary'],
                        ['education', 'Education', "e.g., High School, Bachelor's, Master's"],
                      ] as [keyof Demographics, string, string][]).map(([field, label, placeholder]) => (
                        <div key={field}>
                          <label className="text-[10px] text-muted-foreground font-semibold mb-1 block">{label}</label>
                          <Input
                            value={getDemo(selectedJuror)[field]}
                            onChange={(e) => updateDemographic(selectedJuror, field, e.target.value)}
                            placeholder={placeholder}
                            className="bg-muted border-border text-foreground placeholder-muted-foreground h-8 text-xs"
                          />
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-card rounded-xl border border-border p-3 shadow-xl">
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-sm font-bold text-foreground">Juror Rating</label>
                    </div>
                    <input
                      type="range"
                      min="1"
                    ...
                    />
                    <div className="flex justify-between text-[9px] text-muted-foreground mt-1.5 px-1">
                  ...
                  {/* Strike Management */}
                  <div className="bg-card rounded-xl border border-border p-3 shadow-xl">
                    <h3 className="text-sm font-bold text-foreground mb-2">Strike Management</h3>
                    <div className="space-y-2">
                      {/* Plaintiff Strikes */}
                      <div>
                        <span className="text-[10px] text-muted-foreground uppercase mb-1 block">Plaintiff</span>
                      ...
                      {/* Defense Strikes */}
                      <div>
                        <span className="text-[10px] text-muted-foreground uppercase mb-1 block">Defense</span>
                        <div className="grid grid-cols-2 gap-1.5">
                          <button
                      ...
                      {strike[selectedJuror] && strike[selectedJuror] !== 'none' && (
                        <div className="mt-3 p-3 bg-red-100 dark:bg-red-950/30 border border-red-300 dark:border-red-500/30 rounded-lg">
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <div className="mb-3">
                <h2 className="text-sm font-bold text-foreground tracking-tight mb-1">Compare Jurors</h2>
              ...
              {compareJurors.length > 0 ? (
                ...
                >
                  <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${Math.min(compareJurors.length, 3)}, minmax(0, 1fr))` }}>
                    {compareJurors.sort((a, b) => a - b).map(jurorId => {
                      ...
                      return (
                        <div key={juror.id} className="bg-muted rounded-xl border border-border overflow-hidden">
                          {/* Juror Header */}
                          <div className="p-3 bg-card border-b border-border relative">
                            <button
                              onClick={() => setCompareJurors(compareJurors.filter(id => id !== juror.id))}
                              className="absolute top-2 right-2 p-1 rounded-full hover:bg-secondary/40 transition-colors"
                            >
                              <X className="w-3 h-3 text-muted-foreground hover:text-foreground" />
                            </button>
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-xs font-bold text-muted-foreground">#{String(juror.id).padStart(3, '0')}</span>
                              <span className="text-sm font-bold text-foreground">{juror.name}</span>
                            </div>
                          ...
                          <div className="p-3 space-y-3 text-xs">
                            {/* Rating Score */}
                            <div>
                              <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2 font-bold">Rating</div>
                              <div className="flex items-center justify-between">
                                <span className="text-[11px] text-muted-foreground">Juror Score</span>
                            ...
                            {/* Approval Status */}
                            <div>
                              <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2 font-bold">Approval</div>
                              <div className="space-y-1">
                                <div className="flex items-center justify-between text-[11px]">
                                  <span className="text-muted-foreground">Lead Counsel</span>
                                  <span className={approvals[juror.id]?.lead ? 'text-emerald-400' : 'text-muted-foreground'}>
                                    {approvals[juror.id]?.lead ? '✓ Approved' : '○ Pending'}
                                  </span>
                                </div>
                                <div className="flex items-center justify-between text-[11px]">
                                  <span className="text-muted-foreground">Co-Counsel</span>
                                  <span className={approvals[juror.id]?.second ? 'text-emerald-400' : 'text-muted-foreground'}>
                            ...
                            {strike[juror.id] && strike[juror.id] !== 'none' && (
                              <div>
                                <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2 font-bold">Strike</div>
                                <div className="flex items-center gap-2">
                            ...
                            {jurorMessages.length > 0 ? (
                              <div>
                                <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2 font-bold">Team Messages ({jurorMessages.length})</div>
                                <div className="space-y-1 max-h-32 overflow-y-auto">
                                  {jurorMessages.slice(-4).map((msg, idx) => (
                                    <div key={idx} className="text-[10px] bg-muted/50 rounded p-2 border-l-2 border-amber-500/30">
                                      <div className="font-semibold text-muted-foreground mb-0.5">{msg.sender}</div>
                              ...
                              <div className="text-center py-4 text-muted-foreground text-[11px]">No team messages yet</div>
                            )}
                          </div>
                        </div>
                      );
                ...
                <div
                  className="flex-1 flex flex-col items-center justify-center text-muted-foreground border-2 border-dashed border-border rounded-lg"
                  onDragOver={(e) => {
                    e.preventDefault();
                    e.dataTransfer.dropEffect = 'copy';
                  }}
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground">Click jurors in the grid or drag them here to compare</p>
                </div>
              )}
            </div>
          ) : null}
      ...
      {settingsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <motion.div
            ...
            className="bg-card rounded-2xl shadow-2xl border border-border w-full max-w-2xl max-h-[85vh] overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-border">
              <div className="flex items-center gap-2">
                <Settings className="w-5 h-5 text-amber-400" />
                <h2 className="text-xl font-bold text-foreground">Case Settings</h2>
              </div>
              <button
              ...
              <div className="space-y-4">
                ...
                <div className="bg-muted rounded-xl p-4 border border-border">
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Case Name</label>
                      <Input
                        value={caseName}
                        onChange={(e) => setCaseName(e.target.value)}
                        className="bg-muted/30 border-border text-foreground placeholder-muted-foreground focus:ring-amber-500/50 focus:border-amber-500/50"
                ...
                <div className="bg-muted rounded-xl p-4 border border-border">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Columns</label>
                      <Input
                        type="text"
                    ...
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Rows</label>
                      <Input
                        type="text"
                        value={gridRowsInput}
                        onChange={(e) => {
                  ...
                  <div className="mt-4">
                    <label className="text-xs font-semibold text-muted-foreground mb-2 block">Grid Start Position</label>
                      ...
                      <button
                        ...
                        className={`px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
                          gridStartPosition === "top-left"
                            ...
                            : 'bg-secondary text-muted-foreground border border-border hover:border-amber-500/50'
                        }`}
                      >
                        Top Left
                      </button>
                ...
                <div className="bg-muted rounded-xl p-4 border border-border">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Seated Jury Size</label>
                      <Input
                        type="text"
                    ...
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Alternates</label>
                      <Input
                        type="text"
                ...
                <div className="bg-muted rounded-xl p-4 border border-border">
                  ...
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Maximum Per Side</label>
                    <Input
                      type="text"
                      ...
                      className="bg-muted/30 border-border text-foreground placeholder-muted-foreground focus:ring-amber-500/50 focus:border-amber-500/50 w-48"
                    />
                ...
                <div className="bg-muted rounded-xl p-4 border border-border">
                  ...
                  <div className="space-y-3">
                    {teamMembers.map((member, idx) => (
                      <div key={idx} className="flex items-center gap-3">
                        <Input
                          ...
                          className="flex-1 bg-muted/30 border-border text-foreground placeholder-muted-foreground focus:ring-amber-500/50 focus:border-amber-500/50"
                          placeholder="Name"
                        />
                        <Input
                          value={member.role}
                    ...
                    <button
                      ...
                      className="w-full px-4 py-2 rounded-lg bg-secondary border border-border hover:bg-secondary text-muted-foreground font-semibold text-sm transition-colors flex items-center justify-center gap-2"
                    >
            ...
            <div className="flex items-center justify-end gap-3 p-4 border-t border-border bg-muted">
              <Button
                onClick={() => setSettingsOpen(false)}
                variant="outline"
                className="bg-secondary border-border hover:bg-secondary text-muted-foreground font-semibold"
              >
                Cancel
              </Button>
              <Button
      ...
      {/* AI Drawer */}
      {aiOpen && (
        <div className="absolute right-0 top-0 w-[340px] h-full bg-card border-l shadow-xl p-4 overflow-y-auto">
          ...
          {!selectedJuror ? (
            <p className="text-muted-foreground">Select a juror to generate context-based suggestions.</p>
          ) : aiLoading ? (
            <p className="text-muted-foreground">Analyzing juror context…</p>
          ) : aiError ? (
            ...
            <ul className="space-y-3">
              {aiSuggestions.map((s, i) => (
                <li key={i} className="border rounded p-2 bg-muted">
                  <div className="text-sm mb-2 text-foreground">{s}</div>
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2 pointer-events-none">
        {notifications.map((notif) => {
          const bgColors = {
            normal: 'from-card to-secondary border-border',
